...
int sql_evaluate_primitives(int primitive)
{
  ...
  if (config.sql_optimize_clauses) {
    ...
    if (config.what_to_count_2 & COUNT_VERSION) what_to_count_2 |= COUNT_VERSION;
  ...
  if (what_to_count_2 & COUNT_VERSION) {
    int use_copy=0;
    ...
    strncat(insert_clause, "version", SPACELEFT(insert_clause));
    strncat(where[primitive].string, "version=%u", SPACELEFT(where[primitive].string));
    strncat(values[primitive].string, "%u", SPACELEFT(values[primitive].string));
    values[primitive].handler = where[primitive].handler = count_version_handler;
    values[primitive].type = where[primitive].type = COUNT_INT_VERSION;
    primitive++;
  }
