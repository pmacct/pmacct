...
void p_kafka_set_topic(struct p_kafka_host *kafka_host, char *topic)
{
  if (kafka_host && kafka_host->rk)
  {
    ...
    if (kafka_host->topic)
      p_kafka_unset_topic(kafka_host);
...
{
  if (kafka_host && strlen(kafka_host->broker))
    return kafka_host->broker;
...
void p_kafka_set_broker(struct p_kafka_host *kafka_host, char *host, int port)
{
  ...
  if (strchr(host, ','))
    multiple_brokers = TRUE;

  if (kafka_host && kafka_host->rk)
  {
    /* if host is a comma-separated list of brokers, assume port is part of the definition */
    if (multiple_brokers)
      snprintf(kafka_host->broker, SRVBUFLEN, "%s", host);
    else
    {
      if (host && port)
        snprintf(kafka_host->broker, SRVBUFLEN, "%s:%u", host, port);
      else if (host && !port)
        snprintf(kafka_host->broker, SRVBUFLEN, "%s", host);
    }
    ...
    if ((ret = rd_kafka_brokers_add(kafka_host->rk, kafka_host->broker)) == 0)
    {
      ...
      Log(LOG_DEBUG, "DEBUG ( %s/%s ): %u broker(s) successfully added.\n", config.name, config.type, ret);
  }
}
...
{
  if (kafka_host)
    kafka_host->content_type = content_type;
}

int p_kafka_get_content_type(struct p_kafka_host *kafka_host)
{
  if (kafka_host)
    return kafka_host->content_type;
...
void p_kafka_set_partition(struct p_kafka_host *kafka_host, int partition)
{
  ...
  {
    if (!partition)
      kafka_host->partition = RD_KAFKA_PARTITION_UA;
    else if (partition == FALSE_NONZERO)
      kafka_host->partition = 0;
    else
      kafka_host->partition = partition;
  }
}
...
void p_kafka_apply_global_config(struct p_kafka_host *kafka_host)
{
  ...
  if (kafka_host && kafka_host->config_file && kafka_host->cfg)
  {
    if ((file = fopen(kafka_host->config_file, "r")) == NULL)
    {
      ...
      Log(LOG_INFO, "INFO ( %s/%s ): [%s] Reading librdkafka global config.\n", config.name, config.type, kafka_host->config_file);
...
void p_kafka_apply_topic_config(struct p_kafka_host *kafka_host)
{
  ...
  if (kafka_host && kafka_host->config_file && kafka_host->topic_cfg)
  {
    if ((file = fopen(kafka_host->config_file, "r")) == NULL)
    {
      ...
      Log(LOG_DEBUG, "DEBUG ( %s/%s ): [%s] Reading librdkafka topic configuration.\n", config.name, config.type, kafka_host->config_file);
...
int p_kafka_connect_to_produce(struct p_kafka_host *kafka_host)
{
  if (kafka_host)
  {
    ...
    if (config.debug)
      rd_kafka_set_log_level(kafka_host->rk, LOG_DEBUG);
  }
  else
    return ERR;
...
int p_kafka_produce_data_to_part(struct p_kafka_host *kafka_host, void *data, size_t data_len, int part, int do_free)
{
  ...
  char *kk_topic = p_kafka_get_topic(kafka_host);
  int dump_msg;
  int dump_queue;
  ...
  if (kafka_host && kafka_host->rk && kafka_host->topic && dump_msg)
  {
    ...
    Log(LOG_INFO, "INFO (  ): kafka return%d\n", ret);
  }
...
int dump_the_queue(struct p_kafka_host *kafka_host, int part, int do_free, int dump_queue)
{
  ...
  if (kafka_host && kafka_host->rk && kafka_host->topic && dump_queue)
  {
    ...
    Log(LOG_INFO, "INFO (  ): Finish dumping message in the queue:%s\n", config.type);
  }
...
void p_kafka_close(struct p_kafka_host *kafka_host, int set_fail)
{
  if (kafka_host && !validate_truefalse(set_fail))
  {
    ...
    else
    {
      ...
      if (kafka_host->rk)
        p_kafka_check_outq_len(kafka_host);
    }
...
int p_kafka_check_outq_len(struct p_kafka_host *kafka_host)
{
  ...
  if (kafka_host->rk)
  {
    while ((outq_len = rd_kafka_outq_len(kafka_host->rk)) > 0)
    {
      ...
      {
        if (retries < PM_KAFKA_OUTQ_LEN_RETRIES)
          retries++;
        else
        {
...
{
  if (config.debug)
    Log(LOG_DEBUG, "DEBUG ( %s/%s ): write_and_free_json_kafka(): JSON object not created due to missing --enable-jansson\n", config.name, config.type);
}
#endif
