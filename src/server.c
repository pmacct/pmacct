		...
		struct acc *src, pm_cfgreg_t w, pm_cfgreg_t w2, struct extra_primitives *extras)
{
  ...
  if (w2 & COUNT_SRC_HOST_LON) memcpy(&d1->src_ip_lon, &s1->src_ip_lon, sizeof(d1->src_ip_lon)); 
  if (w2 & COUNT_DST_HOST_LON) memcpy(&d1->dst_ip_lon, &s1->dst_ip_lon, sizeof(d1->dst_ip_lon)); 
#endif
