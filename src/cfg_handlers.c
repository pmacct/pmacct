...
int cfg_key_aggregate(char *filename, char *name, char *value_ptr)
{
  ...
  while (count_token = extract_token(&value_ptr, ',')) {
    ...
    else if (!strcmp(count_token, "seqno")) cfg_set_aggregate(filename, value, COUNT_INT_SEQUENCE_NUMBER, count_token);
    else if (!strcmp(count_token, "version")) cfg_set_aggregate(filename, value, COUNT_INT_VERSION, count_token);
    else {
