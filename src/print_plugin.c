  ...
  for (j = 0; j < index; j++) {
	...
	if (config.what_to_count_2 & COUNT_NDPI_CLASS) {
  	...
  	if (strlen(dst_mac))
        ...
        if (config.what_to_count_2 & COUNT_IN_VLAN) fprintf(f, "%-8u  ", data->in_vlan_id);
	...
	if (config.what_to_count_2 & COUNT_LABEL) P_fprintf_csv_string(f, pvlen, COUNT_INT_LABEL, write_sep(sep, &count), empty_string);
	  ...
	  fprintf(f, "%s%s", write_sep(sep, &count), ndpi_class);
        ...
        if (config.what_to_count_2 & COUNT_IN_VLAN) fprintf(f, "%s%u", write_sep(sep, &count), data->in_vlan_id);
...
void P_write_stats_header_formatted(FILE *f, int is_event)
{
  ...
  if (config.what_to_count_2 & COUNT_IN_VLAN) fprintf(f, "IN_VLAN  ");
...
void P_write_stats_header_csv(FILE *f, int is_event)
{
  ...
  if (config.what_to_count_2 & COUNT_IN_VLAN) fprintf(f, "%sIN_VLAN", write_sep(sep, &count));
