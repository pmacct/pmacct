...
#ifdef WITH_JANSSON
    json_t *obj = voidobj;
    u_int8_t pfx_flags = pnt[0];
    u_int8_t algorithm = pnt[1];
    u_int32_t sid_value;

    /* Flags */
    char flags_hex[8];
    snprintf(flags_hex, sizeof(flags_hex), "0x%02x", pfx_flags);
    json_object_set_new_nocheck(obj, "pfx_sid_flags", json_string(flags_hex));

    /* Algorithm */
    json_object_set_new_nocheck(obj, "pfx_sid_alg", json_integer(algorithm));

    /* SID/Label - distinguish by length */
    if (len == 7) {
      /* 3-octet label (20-bit value) */
      sid_value = ((u_int32_t)pnt[4] << 16) | ((u_int32_t)pnt[5] << 8) | pnt[6];
      json_object_set_new_nocheck(obj, "pfx_sid_label", json_integer(sid_value));
    }
    else if (len == 8) {
      /* 4-octet index */
      sid_value = ((u_int32_t)pnt[4] << 24) | ((u_int32_t)pnt[5] << 16) |
      ((u_int32_t)pnt[6] << 8) | pnt[7];
      json_object_set_new_nocheck(obj, "pfx_sid_index", json_integer(sid_value));
    }
#endif
  }
