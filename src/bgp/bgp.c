    ...
    */ 
    ...
    if (FD_ISSET(config.bgp_sock, &read_descs)) {
	  ...
	  */
	    ...
	    if (!log_notification_isset(&log_notifications.bgp_peers_throttling, NULL, FALSE, FALSE)) {
              Log(LOG_INFO, "INFO ( %s/core/BGP ): throttling at BGP peer #%u\n", config.name, peers_idx);
	      log_notification_set(&log_notifications.bgp_peers_throttling, NULL, FALSE);
	    }
