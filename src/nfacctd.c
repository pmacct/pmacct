...
int main(int argc, char **argv, char **envp)
{
  ...
  // struct p_redis_host redis_host1;
#endif
  ...
  if (config.proc_priority)
  {
    ...
    if (ret)
      Log(LOG_WARNING, "WARN ( %s/core ): proc_priority failed (errno: %d)\n", config.name, errno);
    else
      Log(LOG_INFO, "INFO ( %s/core ): proc_priority set to %d\n", config.name, getpriority(PRIO_PROCESS, 0));
  }
      ...
      else
      {
        ...
        if (list->cfg.what_to_count & (COUNT_SRC_NET | COUNT_DST_NET | COUNT_SUM_NET | COUNT_SRC_NMASK | COUNT_DST_NMASK | COUNT_PEER_DST_IP))
        {
          if (!list->cfg.nfacctd_net)
          {
            ...
            if (list->cfg.networks_mask)
              list->cfg.nfacctd_net |= NF_NET_STATIC;
            if (!list->cfg.nfacctd_net)
  ...
  if (config.pcap_savefile)
    capture_methods++;
  if (config.nfacctd_port || config.nfacctd_ip)
    capture_methods++;
#ifdef WITH_KAFKA
  if (config.nfacctd_kafka_broker_host || config.nfacctd_kafka_topic)
    capture_methods++;
#endif
#ifdef WITH_ZMQ
  if (config.nfacctd_zmq_address)
    capture_methods++;
#endif
  ...
  else
  {
    ...
    if (!config.nfacctd_port)
      config.nfacctd_port = DEFAULT_NFACCTD_PORT;
    ...
    if (rc < 0)
      Log(LOG_ERR, "WARN ( %s/core ): setsockopt() failed for SO_REUSEPORT.\n", config.name);
#endif

    rc = setsockopt(config.sock, SOL_SOCKET, SO_REUSEADDR, (char *)&yes, (socklen_t)sizeof(yes));
    if (rc < 0)
      Log(LOG_ERR, "WARN ( %s/core ): setsockopt() failed for SO_REUSEADDR.\n", config.name);
    ...
    if (config.nfacctd_interface)
    {
      ...
      if (rc < 0)
        Log(LOG_ERR, "WARN ( %s/core ): setsockopt() failed for SO_BINDTODEVICE (errno: %d).\n", config.name, errno);
    }
#endif
    ...
    if (config.nfacctd_ipv6_only)
    {
      ...
      if (rc < 0)
        Log(LOG_ERR, "WARN ( %s/core ): setsockopt() failed for IPV6_V6ONLY.\n", config.name);
    }
  ...
  if (config.nfacctd_allow_file)
    load_allow_file(config.nfacctd_allow_file, &allow);
  else
  ...
  while (list)
  {
    ...
    if (custom_primitives_vlen(&list->cfg.cpptrs))
      list->cfg.data_type |= PIPE_TYPE_VLEN;
    list = list->next;
  }
  ...
  if (config.bgp_daemon)
  {
    ...
    if (config.rpki_roas_file || config.rpki_rtr_cache)
      sleep_time += DEFAULT_SLOTH_SLEEP_TIME;
    sleep(sleep_time);
  }
  ...
  if (config.bmp_daemon)
  {
    ...
    /* Let's give the  thread some advantage to create its structures */
    if (config.rpki_roas_file || config.rpki_rtr_cache)
      sleep_time += DEFAULT_SLOTH_SLEEP_TIME;
    sleep(sleep_time);
  }
  ...
  for (;;)
  {
    ...
    else if (config.nfacctd_kafka_broker_host)
    {
      ...
      case TRUE: /* got data */
        ...
        Log(LOG_INFO, "INFO ( %s/core ): Successful /blocked/ from consuming to kafka\n", config.name);
        if (ret < 0)
    ...
    if (allow.num)
      allowed = check_allow(&allow, (struct sockaddr *)&client);
    if (!allowed)
      continue;
                       ...
                       struct plugin_requests *req, u_int16_t version, struct NF_dissect *tee_dissect)
{
  ...
  if ((count <= V5_MAXFLOWS) && ((count * NfDataV5Sz) + NfHdrV5Sz == len))
  {
    ...
    while (count)
    {
      ...
      if (config.nfacctd_flow_to_rd_map)
        NF_find_id((struct id_table *)pptrs->bitr_table, pptrs, &pptrs->bitr, NULL);
      if (config.bgp_daemon)
        bgp_srcdst_lookup(pptrs, FUNC_TYPE_BGP);
      if (config.bgp_daemon_peer_as_src_map)
        NF_find_id((struct id_table *)pptrs->bpas_table, pptrs, &pptrs->bpas, NULL);
      if (config.bgp_daemon_src_local_pref_map)
        NF_find_id((struct id_table *)pptrs->blp_table, pptrs, &pptrs->blp, NULL);
      if (config.bgp_daemon_src_med_map)
        NF_find_id((struct id_table *)pptrs->bmed_table, pptrs, &pptrs->bmed, NULL);
      if (config.bmp_daemon)
                       ...
                       int *has_templates)
{
  ...
  if (tee_dissect)
  {
    tee_dissect->hdrVersion = version;
    if (version == 9)
      tee_dissect->hdrCount = flowsetNo; /* imprecise .. */
    else if (version == 10)
      tee_dissect->hdrCount = 0;
...
process_flowset:
  ...
  { /* options template: 1 NetFlow v9, 3 IPFIX */
    unsigned char *tpl_ptr = pkt;
    u_int16_t pens = 0;
    ...
    { /* Options coming */
      ...
      while (flowoff + tpl->len <= flowsetlen)
      {
        ...
        if (tpl->tpl[NF9_FLOW_SAMPLER_ID].len || tpl->tpl[NF9_SAMPLING_INTERVAL].len == 4 || tpl->tpl[NF9_SAMPLING_PKT_INTERVAL].len == 4)
        {
          ...
          {
            if (tpl->tpl[NF9_OPT_SCOPE_SYSTEM].len)
          ...
          if (!sentry)
            sentry = create_smp_entry_status_table(&xflow_status_table, entry);
          else
            ssaved = sentry->next;
            ...
            tpl->tpl[NF9_APPLICATION_NAME].len > 0)
        {
          ...
          if (entry)
            centry = search_class_id_status_table(entry->class, class_id);
          if (!centry)
          {
                        ...
                        struct plugin_requests *req)
{
  ...
  if (req->ptm_c.exec_ptm_dissect)
    nfv = ((struct struct_header_v5 *)pkt)->version;
  else
  ...
  case 5:
    ...
    if (!req->ptm_c.exec_ptm_dissect)
      nfv5_check_status(pptrs); /* stats collection */
    break;
  case 9:
...
void NF_evaluate_flow_type(struct flow_chars *flow_type, struct template_cache_entry *tpl, struct packet_ptrs *pptrs)
{
  ...
  if (tpl->tpl[NF9_NAT_EVENT].len)
    ret = NF9_FTYPE_NAT_EVENT;

  /* NetFlow/IPFIX option final override */
  if (tpl->template_type == 1)
    ret = NF9_FTYPE_OPTION;
...
u_int16_t NF_evaluate_direction(struct template_cache_entry *tpl, struct packet_ptrs *pptrs)
{
  ...
  if (tpl->tpl[NF9_DIRECTION].len && *(pptrs->f_data + tpl->tpl[NF9_DIRECTION].off) == 1)
    ret = DIRECTION_OUT;
...
void NF_process_classifiers(struct packet_ptrs *pptrs_main, struct packet_ptrs *pptrs, unsigned char *pkt, struct template_cache_entry *tpl)
{
  if (tpl->tpl[NF9_APPLICATION_ID].len == 2 || tpl->tpl[NF9_APPLICATION_ID].len == 3 || tpl->tpl[NF9_APPLICATION_ID].len == 5)
  {
    ...
    memcpy(&class_id, (pkt + tpl->tpl[NF9_APPLICATION_ID].off + 1), (tpl->tpl[NF9_APPLICATION_ID].len - 1));
    if (entry)
      pptrs->class = NF_evaluate_classifiers(entry->class, &class_id, gentry);
  }
}
...
void nfv9_datalink_frame_section_handler(struct packet_ptrs *pptrs)
{
  ...
  /* XXX: in case of no NF9_DATALINK_FRAME_TYPE, let's assume Ethernet */
    ...
    frame_type = NF9_DL_F_TYPE_ETHERNET;
